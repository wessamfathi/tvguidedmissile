...
void ATGMCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindTouch(IE_Pressed, this, &ATGMCharacter::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &ATGMCharacter::TouchStopped);
...
{
	// If TGM is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in TGM.Build.cs is not automatically propagated
...
void ATGMCharacter::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void ATGMCharacter::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}
